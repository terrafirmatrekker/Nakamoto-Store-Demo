import { BurnAndReleaseTransaction, DepositCommon, RenNetwork } from "@renproject/interfaces";
export declare type AllBurnTransactions<BurnType, ReleaseType> = ConfirmedBurnTransaction<BurnType> | ReleasedBurnTransaction<BurnType> | BurnTransaction | CompletedBurnTransaction<BurnType, ReleaseType>;
export declare const isBurnConfirmed: <X, Y>(x: AllBurnTransactions<X, Y>) => x is ConfirmedBurnTransaction<X>;
...
export declare const isReleased: <X, Y>(x: AllBurnTransactions<X, Y>) => x is ReleasedBurnTransaction<X>;
export interface ReleasedBurnTransaction<BurnType> extends ConfirmedBurnTransaction<BurnType> {
    /**
...
export declare const isBurnCompleted: <X, Y>(x: AllBurnTransactions<X, Y>) => x is CompletedBurnTransaction<X, Y>;
export interface CompletedBurnTransaction<BurnType, ReleaseType> extends ReleasedBurnTransaction<BurnType> {
    /**
...
export interface ErroringBurnSession<DepositType, CustomParams = {}> extends BurnSession<DepositType, CustomParams> {
    /**
     * Latest error detected during processing
     */
    error: Error;
}
export declare const isBurnErroring: <X, Y>(x: ErroringBurnSession<X, {}> | BurnSession<X, Y, {}>) => x is ErroringBurnSession<X, {}>;
