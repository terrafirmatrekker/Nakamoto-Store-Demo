...
import { isConfirming, isOpen, } from "../types/mint";
...
export const renLockAndMint = async (context) => {
...
const initMinter = async (context, callback) => {
    const minter = await renLockAndMint(context);
    if (isOpen(context.tx) &&
...
const handleSettle = async (sourceTxHash, deposit, callback) => {
    try {
...
const handleSign = async (tx, deposit, callback) => {
    var _a, _b;
    try {
...
const handleMint = async (sourceTxHash, deposit, callback, params) => {
    try {
        ...
        const handle = async () => {
            switch (event.type) {
                ...
                case "SIGN":
                    ...
                    break;
                case "MINT":
                    await handleMint(event.hash, deposit, callback, event.params);
                    break;
            }
        };
        handle()
...
const spawnDepositMachine = (machineContext, name) => spawn(buildDepositMachine()
...
export const buildMintConfig = () => ({
    ...
    actions: {
        broadcast: actions.pure((ctx, event) => {
            return Object.values(ctx.depositMachines || {}).map((m) => send(event, { to: m.id }));
        }),
        forwardEvent: send((_, b) => {
            return b;
        }, {
